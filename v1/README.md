...
Basic file-based key-value store. Stores everything in a plain text file (db.txt) with one key:value pair per line.
It lacks any of the optimizations that we expect from a KV store nowadays, just data persistence storing it in-disk.
